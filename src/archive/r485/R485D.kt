...
fun solvePow(a: String): Int {
    if (a == "1") return 1

    val frontDigs = min(a.length, 16)
    val estP3 =
        (((ln(a.substring(0, frontDigs).toDouble()) + ln(10.0) * (a.length - frontDigs)) / ln(3.0)).toInt() - 1)
            .coerceAtLeast(0)

    val n = sizeOfCPoly(a.length) + 5
    val p = a.toCPoly(n)
    val q = fastPow(3, estP3, n)
    val t = CPoly(n)
    var p3 = estP3
...
// a ** b
fun fastPow(a: Int, b: Int, n: Int): CPoly {
    val p = a.toCPoly(n)
    val r = 1.toCPoly(n)
    var pow = b
    while (pow != 0) {
